...
| «a variação do dia diz se o preço está andando» | Diz como o papel está hoje, que é outra pergunta. Entre uma volta do polling e a seguinte um número mudava e nada dizia isso — e o caso interessante é justamente o desencontro: `▼ 18,17` com `+2,66%` ao lado. A seta é colada no número e pintada por `ui::com_seta`, porque colada ela **não pode** herdar a cor da célula. |
| «meia hora sem responder e a fonte melhor cedeu o lugar» | Vale para preço de mercado e é falso para o que é publicado uma vez por dia. O preço do Tesouro nasce com horas de idade — é o fechamento do pregão anterior — e o informado, gravado na importação, é sempre mais novo pelo relógio: a fonte boa aparecia e **voltava para «manual» sozinha** na volta seguinte. O prazo passou a depender da qualidade da origem, saindo de `store::validade`, que já respondia a mesma pergunta para o cache. |
| «um CSV de catorze megas não serve de fonte» | Serve, se vier ordenado. O do Tesouro Transparente é o histórico inteiro desde 2002, mas do **mais recente para o mais antigo**: os cinquenta e oito títulos de hoje são as primeiras cinquenta e oito linhas, e cabem em quatro kilobytes. O servidor anuncia `Accept-Ranges` e **ignora o `Range`**, então quem corta é o cliente, fechando a conexão — `feed::get_ate`. Medido: 0,28 s. |
| «o Yahoo serve qualquer símbolo que ele mostra» | Serve, mas nem todo símbolo é uma série coerente. `TIO=F`, o minério de ferro, respondia 161,91 na chamada de um dia e terminava em 99,37 na de cinco: é um **futuro**, e futuro rola de contrato. O mesmo com `2YY=F`, o Treasury de 2 anos. Os dois saíram; o de 2 anos voltou pelo FRED, que publica a taxa e não um contrato sobre ela, e o minério ficou de fora em vez de entrar com um número que não se sustenta. |
| «rodízio justo distribui a busca» | Distribui, e é o errado quando há linha vazia. O Yahoo busca quatro por volta para não ser bloqueado, e o rodízio revisitava quem já tinha valor antes de completar quem não tinha — o último indicador levava dez minutos, o que é indistinguível de quebrado. Enquanto houver buraco, a volta inteira vai para ele. |
| «coluna vazia é fonte sem o dado» | Era fonte sem o *campo*. A Kinvo não publica máxima nem mínima prontas, mas publica a série de cinco minutos — e o máximo dela é o máximo do dia. Volume ela não publica em endereço nenhum, e aí o acessório passou a ser herdado de outra fonte da mesma volta, nunca de outro pregão. |
| «uma grade de fileiras iguais mostra o peso» | Mostra a **largura**, não a área. Com fileiras de `n` células e altura fixa, uma posição de 0,5% saía do tamanho de uma de 3% — e havia ainda um piso de oito colunas que engordava justamente as menores. O heatmap virou um treemap *squarified* de verdade, sem piso: quem não cabe legível sai do mapa em vez de sair do tamanho errado. |
| «abreviar número é perder informação» | Cortar é pior. `R$ 30.291,00` cortado na largura da caixa dá `R$ 30.29`, que não é um número truncado — é outro número, e se lê como trinta reais. `calc::moeda_curta` diz `30,3 mil`, e quando nem isso cabe a linha some. |
| «o feed carimba em ISO ou em RFC 822» | Quatro dos cinco endereços do investing.com carimbam `2026-09-10 12:08:07` — espaço no lugar do `T`, mês em número, sem fuso. Não é nenhum dos dois formatos, e **215 de 240 notícias ficavam sem data** por causa disso, indo para o fim de uma lista ordenada por data. O leitor aceita espaço e data sem hora, e um carimbo sem fuso vale UTC — medido contra o relógio, não chutado. |
| «ordenar notícia não precisa de fuso» | Era a justificativa escrita para `epoch_de_iso` jogar o `+03:00` fora. Precisa: três horas de erro põem a notícia da manhã depois da notícia da tarde, que é exatamente a ordem que a lista existe para acertar. |
| «o carimbo do dado responde a defasagem» | São duas perguntas. «De quando é este preço» é o carimbo do dado — e com a bolsa fechada ele tem dezesseis horas, o que está certo. «Há quanto tempo o programa falou com a fonte» é outra coisa, e é ela que separa «o mercado está parado» de «o programa parou». A segunda não existia em lugar nenhum; agora é a tabela `busca`, uma linha por fonte, gravada só no sucesso. |
